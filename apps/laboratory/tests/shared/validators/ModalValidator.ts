...
export class ModalValidator {
  ...
  async expectReownExtensionOption() {
    // Check for Reown extension in injected wallets
    const reownExtension = this.page.getByTestId('wallet-button-Reown')
    const isVisible = await reownExtension.isVisible()
    ...
    // If not visible, that's also acceptable as it depends on extension installation
  }

  async expectHeadlessQRCode() {
    const qrCode = this.page.locator('svg[data-testid="qr-code"]')
    await expect(qrCode, 'QR code should be visible').toBeVisible({
      timeout: MAX_WAIT
    })
  }

  async expectValidWalletConnectURI() {
    const qrCode = this.page.locator('svg[data-testid="qr-code"]')
    await expect(qrCode, 'QR code should be visible').toBeVisible({
      timeout: MAX_WAIT
    })

    // Check that the QR code has a valid URI value
    const uri = await qrCode.getAttribute('data-value')
    expect(uri, 'QR code should have a valid URI').toBeTruthy()
    expect(uri, 'URI should start with wc:').toMatch(/^wc:/)
  }

  async expectHeadlessSearchView() {
    const searchHeading = this.page.getByText(/Search Wallets \(\d+\)/)
  ...
  async expectOtherWalletsHidden() {
    // Check that non-MetaMask wallets are hidden
    const walletItems = this.page.getByTestId(/^wc-wallet-item-(?!MetaMask)/)
    const count = await walletItems.count()

    for (let i = 0; i < count; i++) {
      const item = walletItems.nth(i)
      await expect(item, `Wallet item ${i} should be hidden`).toBeHidden()
    }
  }

  async expectAllWalletsVisible() {
    // Check that we have multiple wallet items visible
    const walletItems = this.page.getByTestId(/^wc-wallet-item-/)
    const count = await walletItems.count()
    expect(count, 'Should have multiple wallets visible').toBeGreaterThan(1)
  }
  ...
  async expectSearchInputValue(expectedValue: string) {
    const searchInput = this.page.getByPlaceholder('Search by name...')
    await expect(searchInput, `Search input should have value "${expectedValue}"`).toHaveValue(
      expectedValue
    )
  }
  ...
  async expectWalletCountChanged(initialCount: number, newCount: number) {
    expect(newCount, 'Wallet count should have increased').toBeGreaterThan(initialCount)
  }

  async expectNoWalletsFound() {
    const noWalletsMessage = this.page.getByText('No wallets found')
    await expect(noWalletsMessage, 'No wallets found message should be visible').toBeVisible({
      timeout: MAX_WAIT
    })
  }

  async expectWalletItemVisible(walletName: string) {
    const cleanWalletName = walletName.replace(/\s+/g, '')
    const walletItem = this.page.getByTestId(`wc-wallet-item-${cleanWalletName}`)
    await expect(walletItem, `${walletName} should be visible`).toBeVisible({
      timeout: MAX_WAIT
    })
  }

  async expectWalletItemHidden(walletName: string) {
    const cleanWalletName = walletName.replace(/\s+/g, '')
    const walletItem = this.page.getByTestId(`wc-wallet-item-${cleanWalletName}`)
    await expect(walletItem, `${walletName} should be hidden`).toBeHidden()
  }
}
