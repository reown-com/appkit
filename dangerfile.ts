...
async function checkUiPackage() {
  ...
  const types_util_index = modified_files.find(f => f.includes('ui/src/utils/JSXTypesUtil.ts'))
  const types_util = types_util_index ? await diffForFile(types_util_index) : undefined
  ...
  if (created_ui_components_index_ts.length && !types_util?.added.includes('../components')) {
    fail(
  ...
  if (is_new_layout_added && !types_util?.added.includes('../layout')) {
