...
// -- presets ------------------------------------------------------ //
export const injectedPreset: Record<InjectedId, Preset> = {
  [InjectedId.metaMask]: {
    name: 'MetaMask',
...
export function getInjectedId(id: string): InjectedId {
  if (id.toUpperCase() !== 'INJECTED') return id as InjectedId
  ...
  return 'injected' as InjectedId
}

export function getInjectedName(name: string) {
  if (name.toUpperCase() !== 'INJECTED' && name !== 'MetaMask') {
    return name
  }
  const id = getInjectedId('')

  return injectedPreset[id].name
}
