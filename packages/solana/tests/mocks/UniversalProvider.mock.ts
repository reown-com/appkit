...
export const mockUniversalProvider = (): UniversalProvider => {
...
const mockSession = (replaces: Partial<SessionTypes.Struct> = {}): SessionTypes.Struct => {
  const chains = TestConstants.chains.map(chain => `solana:${chain.chainId}`)

  const accounts = chains.reduce((acc, cur) => {
