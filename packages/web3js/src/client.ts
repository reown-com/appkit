...
import { ensReverse, getBalance } from './utils/helpers.js'
...
export class Web3Modal extends Web3ModalScaffold {
  ...
  public constructor(options: Web3ModalClientOptions) {
    const { web3Config, chains, defaultChain, tokens, chainImages, _sdkVersion, ...w3mOptions } =
      options
    ...
    super({
      ...
      _sdkVersion: _sdkVersion ?? `html-web3js-${ConstantsUtil.VERSION}`,
      ...w3mOptions
    })
  ...
  private async syncBalance(address: Address) {
    ...
    if (provider && chainId && this.chains) {
      const chain = this.chains.find(c => c.chainId === chainId)

      if (chain) {
        const formattedBalance = await getBalance({ address, provider })
