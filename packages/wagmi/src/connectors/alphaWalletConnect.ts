...
alphaWalletConnect.type = 'walletConnect' as const
export function alphaWalletConnect(parameters: WalletConnectParameters) {
  ...
  return createConnector<Provider, Properties, StorageItem>(config => ({
    ...
    getNamespaceChainsIds() {
      if (!provider_) return []
      const chainIds = provider_.session?.namespaces[NAMESPACE]?.chains?.map(chain =>
