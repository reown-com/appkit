...
export const ChainController = {
  ...
  onSwitchNetwork(network: CaipNetwork) {
    const currentNetwork = state.activeCaipNetwork
     ...
     */
    const isConnectedWithAuth = connectorId === CommonConstantsUtil.CONNECTOR_ID.AUTH
    const isSupportedForAuthConnector = CommonConstantsUtil.AUTH_CONNECTOR_SUPPORTED_CHAINS.find(
