...
export const ChainController = {
  ...
  initialize(adapters: ChainAdapter[]) {
    ...
    const nonUniversalAdapters = adapters?.filter(a => a.adapterType !== 'universal') ?? []
    // eslint-disable-next-line no-console
    console.log('additionalProviders', nonUniversalAdapters)

    if (nonUniversalAdapters.length === 0 || !adapterToActivate) {
