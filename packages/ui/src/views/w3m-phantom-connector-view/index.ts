...
@customElement('w3m-phantom-connector-view')
export class W3mPhantomConnectorView extends LitElement {
  ...
  // -- private ------------------------------------------------------ //
  private readonly phantomUrl = 'https://phantom.app'

  private async onConnect() {
    try {
      this.error = false
      this.connecting = true
      await ClientCtrl.solana().connectPhantom()
  ...
  private onInstall() {
    CoreHelpers.openHref(this.phantomUrl, '_blank')
  }
    ...
    return html`
      <div class="w3m-connecting-title">
        ...
        <w3m-text variant="large-bold" color=${this.error ? 'error' : 'secondary'}>
          ${this.error ? 'Connection declined' : 'Continue in Phantom...'}
        </w3m-text>
      </div>
      <w3m-button
  ...
  private notReadyTemplate() {
    return html`
      <div class="w3m-install-title">
        <w3m-text variant="large-bold">Install Phantom</w3m-text>
        <w3m-text color="secondary" align="center" variant="medium-thin">
          To connect Phantom wallet, install the browser extension.
        </w3m-text>
      </div>
  ...
  // -- render ------------------------------------------------------- //
  protected render() {
    const ready = ClientCtrl.solana().getConnectorIsAvailable('injected-window.phantom.solana')
    const classes = {
    ...
    return html`
      <w3m-modal-header title="Phantom"></w3m-modal-header>
      <w3m-modal-content>
        <div class=${classMap(classes)}>
          <w3m-wallet-image name="Phantom" size="lg"></w3m-wallet-image>
...
declare global {
  interface HTMLElementTagNameMap {
    'w3m-phantom-connector-view': W3mPhantomConnectorView
  }
}
