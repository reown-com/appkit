...
import { ConstantsUtil, ErrorUtil, WalletConnectLoggerUtil, PresetsUtil } from '@reown/appkit-utils'
...
export class UniversalAdapterClient {
  ...
  private async initWalletConnectProvider(projectId: string) {
    const logger = WalletConnectLoggerUtil.createLogger((err, ...args) => {
      if (err.message.includes(ErrorUtil.UniversalProviderErrors.UNAUTHORIZED_DOMAIN_NOT_ALLOWED)) {
        ...
        // eslint-disable-next-line no-console
        console.error(
          ErrorUtil.ALERT_ERRORS.ORIGIN_NOT_WHITELISTED.replace('{{origin}}', window.origin)
        )
