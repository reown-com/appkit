...
export class EVMWagmiClient {
  private protocol: 'evm'

  private scaffold: Web3ModalScaffold
  ...
  public getCaipDefaultChain = getCaipDefaultChain(this.options?.defaultChain)

  public _sdkVersion = this.options?._sdkVersion || `html-wagmi-${ConstantsUtil.VERSION}`
  ...
  public networkControllerClient: NetworkControllerClient = {
    ...
    getApprovedCaipNetworksData: async () => {
      const wagmiConfig = this.options?.wagmiConfig
    ...
    getEnsAddress: async (value: string) => {
      try {
        ...
        if (value?.endsWith(CommonConstants.WC_NAME_SUFFIX)) {
          wcName = await this.scaffold.resolveWalletConnectName(value)
        }
  ...
  private async listenAuthConnector(
    ...
    if (typeof window !== 'undefined' && connector) {
      ...
      provider.onIsConnected(req => {
        ...
        this.scaffold.setPreferredAccountType(req.preferredAccountType as W3mFrameTypes.AccountType)
        this.scaffold.setLoading(false)
      })
